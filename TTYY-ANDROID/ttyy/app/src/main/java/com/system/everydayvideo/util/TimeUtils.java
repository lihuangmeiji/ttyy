...
public class TimeUtils {
    ...
    /**
     * 将原格式为yyyy-MM-dd HH:mm:ss的时间格式字符串转换成yyyy.MM.dd格式
     */
    public static String string2String(String time) {
        return millis2String(string2Millis(time),"yyyy.MM.dd");
    }
    /**
     * 将时间字符串转为Date类型
     * <p>time格式为yyyy-MM-dd HH:mm:ss</p>
     ...
     * 判断是否为同一个月
     */
    public static boolean isSameMonth(long mills) {
       return TextUtils.equals(millis2String(mills, "yyyy年MM月"),millis2String(System.currentTimeMillis(),"yyyy年MM月"));
    }
    ...
    public static boolean isSameDay(long millis,long mills2) {
        Date date1 = new Date(millis);
        Date date2 = new Date(mills2);
     ...
     * @return
     */
    public static List<String> getLastMonths(int num , String format){
        List<String> list = new ArrayList<>();
        SimpleDateFormat matter=new SimpleDateFormat(format);
        Calendar cal = new GregorianCalendar();
        cal.add(Calendar.MONTH, 1); //要先+1,才能把本月的算进去</span>
        for(int i=0; i<num; i++){
            cal.add(Calendar.MONTH, -1); //逐次往前推1个月
            Date data = cal.getTime();
            String string = matter.format(data);
            list.add(string);
        }
        return list;
    }
