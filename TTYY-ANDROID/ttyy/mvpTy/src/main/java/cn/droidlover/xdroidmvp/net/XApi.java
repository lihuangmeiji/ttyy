...
public class XApi {
    ...
    private static  String hspd=null;
    ...
    public static void registerProvider(Context context,String hspd,NetProvider provider) {
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider) {
        ...
        Https.SSLParams sslParams= Https.getSslSocketFactory(null,hspd,context);
     ...
     * @return
     */
    public static <T extends IResponse> FlowableTransformer<T, T> getScheduler() {
     ...
     * @return
     */
    public static <T extends IResponse> FlowableTransformer<T, T> getApiTransformer() {
                ...
                return upstream.flatMap(new Function<T, Publisher<T>>() {
                    ...
                    public Publisher<T> apply(T model) throws Exception {
                        if (model.getCode().equals("1")) {
                            return Flowable.error(new NetError(model.getMsg(), NetError.BusinessError));
                        } else if (model.getCode().equals("-1")) {
                            return Flowable.error(new NetError(model.getMsg(), NetError.AuthError));
                        }else if(model.getCode().equals("0")){
                            return Flowable.just(model);
                        }else {
                            return Flowable.error(new NetError(model.getMsg(), NetError.NoConnectError));
                        }
                    }
                });
            }
